 ...
 * Created by Tinghui Liao
 * Forked by, Christopher Jeffrey. (MIT Licensed)
...
inline.gfm = merge({}, inline.normal, {
  ...
  del: /^~+(?=\S)([\s\S]*?\S)~+/,
  toc: /\s*\[TOC\]/, //TOC Match, any black char along with [TOC]
  ...
  if (this.options.headerIds) {
    if(this.options.toc) {
      return '<h' + level + ' id="' + this.options.headerPrefix + raw.toLowerCase().replace(/[^\w]+/g, '-') + '">'
        + '<a href="#' + this.options.headerPrefix + raw.toLowerCase().replace(/[^\w]+/g, '-')  + '">'
        + text
        + '</a></h>\n';
    }
